...
!
      ...
      USE NETCDF 
      USE TIME_MOD              
      USE INPUT_ADJ_MOD, ONLY: OBS_DIR, FILE_PREFIX, FILE_SUFFIX
      IMPLICIT NONE
	...
	write(READ_filename,102)TRIM(tempDIr), !JYun
	!write(READ_filename,102)TRIM(tempDIr),TRIM(FILE_PREFIX),
       !& year, month, day,TRIM(FILE_SUFFIX)
     & day,TRIM(FILE_SUFFIX)
	

101    format(a,i4.4,'/',i2.2,'/')
!102    format(a,a,i4.4,i2.2,i2.2,a)
102    format(a,i2.2,a) !JYun
!102    format(a,a,i2.2,a)
      ...
      CALL CHECK( NF90_INQ_VARID(FID,"xCO2-averagingKernel",AK_ID),104)
      CALL CHECK( NF90_INQ_VARID( FID, "pressure",        PS_ID ), 105 )
      CALL CHECK( NF90_INQ_VARID(FID,"CO2-apriori",AP_ID), 107 )
      CALL CHECK( NF90_INQ_VARID( FID, "latitude",        LA_ID ), 108 )
      CALL CHECK( NF90_INQ_VARID( FID, "longitude",       LO_ID ), 109 )
!      CALL CHECK( NF90_INQ_VARID( FID, "landfrac",       frac_ID ),110 )
      CALL CHECK( NF90_INQ_VARID( FID, "time",            TM_ID ), 111 )
      CALL CHECK(NF90_INQ_VARID(FID,"xCO2-uncertainty",OI_ID)
     &,112) 
      CALL CHECK( NF90_INQ_VARID( FID, "xCO2-apriori", xAP_ID),115) 
      CALL CHECK( NF90_INQ_VARID( FID, "xCO2"
     &,xCO2_ID ), 116)
      ! READ quality flag 
!      CALL CHECK( NF90_INQ_VARID( FID, "mode", QF_ID ), 114 )
      CALL CHECK( NF90_INQ_VARID( FID,"xCO2-pressureWeight"
     &,weight_ID),116)
...
!     &      GOS(N)%QF, START0, COUNT0 ), 305 )
     ...
     &      temp2,    START1, COUNT1 ), 402 )
	  DO ilev=1, J
	     GOS(N)%PRES(ilev)=temp2(ilev)
	  ENDDO
	  

         ! READ apriori CO2 column, PRIOR
         CALL CHECK( NF90_GET_VAR  ( FID, AP_ID, 
     &     temp2,    START1, COUNT1 ), 403 )
	  DO ilev=1, J
	     GOS(N)%PRIOR(ilev)=temp2(ilev)
	  ENDDO


         ! READ xCO2 weighting function
         CALL CHECK( NF90_GET_VAR  ( FID, AK_ID, 
     &    temp2,    START1, COUNT1 ), 404 )
	  DO ilev=1, J
	     GOS(N)%xAK(ilev)=temp2(ilev)
	  ENDDO

         CALL CHECK( NF90_GET_VAR  ( FID, weight_ID, 
     &     temp2,    START1, COUNT1 ), 404 )
	  DO ilev=1, J
	     GOS(N)%weight(ilev)=temp2(ilev)
	  ENDDO
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE CALC_GOSAT_CO2_FORCE( COST_FUNC )
!
!******************************************************************************
...
#     include      "CMN_SIZE"      ! Size params
      ...
      INTEGER                     :: IOS, temp3, mode, landfrac
...
!      DO NT  = NTSTART, NTSTOP, -1
	...
	IF((GC_HH_FRAC > (TIME_FRAC(NT)-0.5/24.0)) 
     & .and. (GC_HH_FRAC < = (TIME_FRAC(NT)+0.5/24.0)))then
         ...
         ! quality screening
!         IF ( GOS(NT)%QF(1) .eq. mode.and.
!     &GOS(NT)%landfrac(1).eq.landfrac)THEN 
         !IF ( GOS(NT)%QF(1) .ne. 1 ) then
         ...
         ! dkh debug  
         !print*, 'I,J = ', I, J, GOS(NT)%LON(1),GOS(NT)%LAT(1)
 ...
 100  FORMAT(1X,F16.8,1X,F20.8)
         ...
         ! x_m - x_a
	!write(6,*)'lon=', GOS(NT)%lon(1),'lat=', GOS(NT)%lat(1)
         DO L = 1, LGOS 
           CO2_PERT(L) = GC_CO2(L) - GOS(NT)%PRIOR(L)
	   !write(6,*)'CO2 diff=', GC_CO2(L),GOS(NT)%PRIOR(L)
         ENDDO
...
!     &                    * CO2_PERT(LL) 
	...
	!write(6,*)'xPrior=', GOS(NT)%xPRIOR, 'CO2_HAT=', CO2_HAT(1)
         ...
         ENDDO 
      !	endif
	!IF (abs(DIFF(1))*1e6.le.6.0)THEN

!	  YYYYMMDD=GET_NYMD() 
!	  write(filenameobs(6:13),'(I8)')YYYYMMDD
!          open(99,file=filenameobs,form='formatted',position='append')
!          write(99, '(F7.2, 2X, F7.2, 2X, F10.4, 2X, F10.4,2X, F7.2)')
!     &  GOS(NT)%lat(1),GOS(NT)%lon(1), CO2_HAT(1)*1e6,GOS(NT)%xCO2*1e6,
!     &  GOS(NT)%TIME(1)*24D0
	!write(*,*)'DIFF(1)=', DIFF(1)*1e6,GOS(NT)%xstd(1)
	
	!
	! OCO2 observation error has some problem
	!
...
!!$OMP END PARALLEL DO
      ...
      ! Return to calling program
      END SUBROUTINE CALC_GOSAT_CO2_FORCE
