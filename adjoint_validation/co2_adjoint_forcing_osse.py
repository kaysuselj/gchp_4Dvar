#!/usr/bin/env python3
"""
Compute 3D adjoint forcing from OCO-2 OSSE xCO2 observations, binned to GCHP
...
Environment setup:
    ...
    The following file must be in the same directory:
        convert_satelite_tracks_osse.py (for reading OSSE observations)
...
Notes:
    ...
    - OSSE OCO-2 data uses ORCHIDEE-ECCO2 synthetic observations.
"""
...
# ---------------------------------------------------------------------------
# Read OSSE observations (monthly aggregation from daily files)
# ---------------------------------------------------------------------------

def read_oco_monthly_osse(year, month, base_fold, day_start=1, day_end=None):
    """
    Read OSSE OCO-2 observations for a given month.
    ...
    month : int
    base_fold : str
        Base folder containing OSSE data
    day_start : int, optional
        First day to read (1-indexed, default 1)
    day_end : int, optional
        Last day to read inclusive (default: last day of month)

    Returns
    -------
    xr.Dataset or None
        Combined dataset for the month, or None if no data found
    """
    ...
    for day in range(day_start, day_end + 1):
        try:
            ...
            print(f"  Warning: Failed to read {year}-{month:02d}-{day:02d}: {e}")
...
def _accumulate_forcing_one_file(gchp_file, checkpoints, obs_by_ckpt, stats,
    ...
    for year, month in year_months:
        ...
        mo_end   = pd.Timestamp(year, month, calendar.monthrange(year, month)[1])
        d_start  = max(mo_start, pd.Timestamp(t_start)).day
        d_end    = min(mo_end,   pd.Timestamp(t_end)).day
        ds_obs = read_oco_monthly_osse(year, month, BASE_FOLD_OBS,
